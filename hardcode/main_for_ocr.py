...
for subfolder in subfolders:
    ...
    pdf_files = [f for f in os.listdir(subfolder_path) if f.lower().endswith(('.jpg','.png'))]
    ...
    if(v<=2):
        ...
        for pdf_file in pdf_files:
            ...
            txtfile = pdf_file.strip('.jpg')
            output_file = os.path.join(subfolder_path, f'{txtfile}.txt')
            with open(output_file, 'w', encoding='utf-8') as output_txt:
                ...
                text = pytesseract.image_to_string(Image.open(pdf_path), lang='eng', config='--psm 6')
            ...
            print(output_file+' sadsad '+subfolder_path+'   '+f'{txtfile}.txt')
            output_files=txt_splitter(output_file,subfolder_path,f'{txtfile}.txt')
            print(output_files)
            time.sleep(3)
